...
::field-group
  ...
  ::field{name=%longtime% type=DateTime}
  Get the current system time (formatted as `hh:mm:ss`)

  Formatting is supported, e.g. `%time:HH-mm%`{lang=cs}
  ::

  ::field{name=%unixtime% type=DateTime}
  Get the current system unix time (formatted as `1725936677`)
  ::

  ::field{name=%filedatetime% type=DateTime}
  Returns when a file was created or last modified (formatted as `yyyyMMdd.hhmmss`)
  *(Only returns properly when Sub-Actions include Read/Write to file)*
  Formatting is supported, e.g. `%time:HH-mm%`{lang=cs}
  ::
