...
To get or set a user variable you must also provide a platform (e.g. `Twitch` or `YouTube`) and a user identifier (e.g. `username` or `id`)

:read-more{to=/api/sub-actions/twitch/globals}
:read-more{to=/api/sub-actions/youtube/globals}
